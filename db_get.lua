Quester_File["db_get.lua"] = "1.4.1"
Quester_Loadtime["db_get.lua"] = GetTime()

local dev_mode = (Quester_File["db_get.lua"] == "Development Version")
...
Quester: Assert(dev_mode or #QHDB == 4)
...
local function DBC_Put(group, id, item)
  ...
  Quester: Assert(not cache[group][id])
  cache[group][id] = item
...
local function read_adaptint(data, offset)
  ...
  while true do
    ...
    Quester: Assert(v, string.format("%d %d", #data, offset))
...
function DB_GetItem(group, id, silent, register)
  ...
  Quester: Assert(group, string.format("%s %s", tostring(group), tostring(id)))
  Quester: Assert(id, string.format("%s %s", tostring(group), tostring(id)))
  local ite = DBC_Get(group, id)
  
  if not ite then
    if type(id) == "string" then Quester: Assert(not id:match("__.*")) end
    
    --Quester:TextOut(string.format("%s %d", group, id))
    ...
    for _, db in ipairs(QHDB) do
      ...
      if db[group] then
        ...
        if dat then
          if not ite then ite = Quester:CreateTable("db") end
          
          local srctab
          
          if type(dat) == "string" then
            Quester: Assert(db[group].__tokens == nil or type(db[group].__tokens) == "table")
          ...
          for k, v in pairs(srctab) do
            Quester: Assert(not ite[k] or k == "used")
            ite[k] = v
          end
        end
      end
    ...
    if ite then
      ...
      if not silent then
        Quester:TextOut(string.format("Tried to get %s/%s, failed", tostring(group), tostring(id)))
      end
    end
  end
...
function DB_ReleaseItem(ite)
  ...
  if frequencies[ite] == 0 then
    ...
    local incin = Quester:CreateTable("incinerate")
    incinerate(ite, incin)
    for k, _ in pairs(incin) do
      Quester:ReleaseTable(k)
    end -- burn baby burn
    Quester:ReleaseTable(incin)
  end
end
...
function DB_ListItems(group)
  ...
  for _, db in ipairs(QHDB) do
    if db[group] then
      Quester: Assert(not db.__serialize_index and not db.__serialize_data)
